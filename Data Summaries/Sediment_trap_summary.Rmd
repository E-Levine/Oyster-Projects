...
```{r Percent organic sumamry and comparisons - estuary and station, include=FALSE}
#
...
Sediment on oyster reefs in the Caloosahatchee River estuary averaged `r round((PctOrgSumm_overall %>% filter(Estuary == "CR" & is.na(Station)))$MeanPct, 1)`% organic content (Table 8, Figure 3). The lowest percent organic content occurred at station `r (PctOrgSumm_overall %>% group_by(Estuary) %>% slice_min(order_by = MeanPct, na_rm = T) %>% filter(Estuary == "CR"))$Station` (mean: `r round((PctOrgSumm_overall  %>% group_by(Estuary) %>% slice_min(order_by = MeanPct, na_rm = T) %>% filter(Estuary == "CR"))$MeanPct,1)`%) and the highest occurred at station `r (PctOrgSumm_overall %>% group_by(Estuary) %>% slice_max(order_by = MeanPct, na_rm = T) %>% filter(Estuary == "CR"))$Station` (mean: `r round((PctOrgSumm_overall  %>% group_by(Estuary) %>% slice_max(order_by = MeanPct, na_rm = T) %>% filter(Estuary == "CR"))$MeanPct,1)`%). Overall, the lowest single month percent organic content (`r round((Month_sedi %>% group_by(Estuary) %>% slice_min(order_by = MeanPctOrg, na_rm = T) %>% filter(Estuary == "CR"))$MeanPctOrg,1)`%) occurred in `r (Month_sedi %>% group_by(Estuary) %>% slice_min(order_by = MeanPctOrg, na_rm = T) %>% filter(Estuary == "CR"))$RetDate %>% format("%B %Y")` at station `r (Month_sedi %>% group_by(Estuary) %>% slice_min(order_by = MeanPctOrg, na_rm = T) %>% filter(Estuary == "CR"))$Station ` while the highest (`r round((Month_sedi %>% group_by(Estuary) %>% slice_max(order_by = MeanPctOrg, na_rm = T) %>% filter(Estuary == "CR"))$MeanPctOrg,1)` g/month) occurred in `r (Month_sedi %>% group_by(Estuary) %>% slice_max(order_by = MeanPctOrg, na_rm = T) %>% filter(Estuary == "CR"))$RetDate %>% format("%B %Y")` at station `r (Month_sedi %>% group_by(Estuary) %>% slice_max(order_by = MeanPctOrg, na_rm = T) %>% filter(Estuary == "CR"))$Station` (Table 2). 
\
\
Oyster reefs in the Tampa Bay estuary had an average sedimentation rate of `r round((SedimentSumm_overall %>% filter(Estuary == "TB" & is.na(Station)))$MeanRate, 1)` g/month (Table 1, Figure 1). The lowest sedimentation rate occurred at station `r (SedimentSumm_overall %>% group_by(Estuary) %>% slice_min(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$Station` (mean: `r round((SedimentSumm_overall  %>% group_by(Estuary) %>% slice_min(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$MeanRate,1)` g/month) and the highest occurred at station `r (SedimentSumm_overall %>% group_by(Estuary) %>% slice_max(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$Station` (mean: `r round((SedimentSumm_overall  %>% group_by(Estuary) %>% slice_max(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "CR"))$MeanRate,1)` g/month). The lowest single station sedimentation rate (`r round((Month_sedi %>% group_by(Estuary) %>% slice_min(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$MeanRate,1)` g/month) occurred in `r (Month_sedi %>% group_by(Estuary) %>% slice_min(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$RetDate %>% format("%B %Y")` at station `r (Month_sedi %>% group_by(Estuary) %>% slice_min(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$Station ` while the highest (`r round((Month_sedi %>% group_by(Estuary) %>% slice_max(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$MeanRate,1)` g/month) occurred in `r (Month_sedi %>% group_by(Estuary) %>% slice_max(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$RetDate %>% format("%B %Y")` at station `r (Month_sedi %>% group_by(Estuary) %>% slice_max(order_by = MeanRate, na_rm = T) %>% filter(Estuary == "TB"))$Station` (Table 2). 
\
