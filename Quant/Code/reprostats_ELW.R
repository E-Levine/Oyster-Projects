...
#ABLog=read.csv("ABLog.csv", header=T) - don't have this file...

wq=read.csv("Data/WQ.csv", header = T, na.strings = "Z")
wq
repro
str(repro)
...
#? not created until L87: str(gonadmean)
...
ggplot(oocytedatasummary, aes(x=Month, y=mean, colour=Site, group=Site)) + 
  ...
  geom_line() +
  geom_point() +
  labs(y= "Average Oocyte Diamater (?m)", x = "Month") +
  scale_x_discrete(limits = month.abb) +
...
oocytedatasummary%>%filter(Site=="AB")%>%
  ...
  geom_line(color="dodgerblue4") +
  geom_point(color="dodgerblue4") +
  labs(y= "Average Oocyte Diameter (?m)", x = "Month") +
  labs(title = "Apalachicola Bay") +
  scale_x_discrete(limits = month.abb) +
  scale_y_continuous(limits=c(0,50), breaks = seq(0,50, by = 10)) +
  theme_classic2()
...
###SH BY STAGE - where does SH come from?
summary(repro$SH)
repro<-na.omit(repro)
...
ggplot(data=oostagedata, aes(x=Stage, y=mean, fill=Site,)) +
  ...
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                ...
                width=.2,
                position=position_dodge(.93)) +
  labs(y= "Average Oocyte Diameter (?m)", x = "Stage") +
...
a <- ggplot(oocytemean, aes(x = Site, y = OocyteDiam)) +
  geom_boxplot() +
  labs(y= "Oocyte Diameter (?m)", x = "Site") +
  scale_y_continuous(limits=c(0,40), breaks = seq(0,40, by = 10)) +
  theme_classic() 
a
